...
public class PlayMakerPhotonProxy : Photon.MonoBehaviour
{
	...
	public void SanitizeGameObject(GameObject go)
	{
		...
		{	
			...
			{
				_comp_i = photonView.ObservedComponents[i];
	...
	*/
	...
	void ProcessCallbackStack()
	{
		...
		Debug.Log("Processing Callback Stack: "+_callback.CallbackEvent+"\nfor player:"+lastMessagePhotonPlayer);
		PlayMakerFSM.BroadcastEvent(_callback.CallbackEvent);
	}
	...
	void OnPhotonPlayerConnected (PhotonPlayer player)
	{
		...
		_c.Player = player;
		_c.CallbackEvent = "PHOTON / PHOTON PLAYER CONNECTED";
	...
	{
		if (debug) {
			Debug.Log ("PlayMaker Photon proxy:OnPlayerDisconnected: " + player);
		}
		
		PunCallbackInfo _c = new PunCallbackInfo();
		_c.Player = player;
		_c.CallbackEvent = "PHOTON / PHOTON PLAYER DISCONNECTED";
